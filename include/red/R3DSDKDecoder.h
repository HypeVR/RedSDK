...
namespace R3DSDK
{
	...
	public:
		...
		static R3DStatus GetOpenCLDeviceList(std::vector<R3DSDK::OpenCLDeviceInfo> &devices);
	    ...
	    //returns a list of opencl devices compatible with SDK
		static R3DStatus GetCudaDeviceList(std::vector<R3DSDK::CudaDeviceInfo> &devices);

	    //sets the cuda device to be used by the decoder
	    //cuda and opencl are mutually exclusive, you cannot combine usage of cuda and opencl on the same options instance
		R3DStatus useDevice(CudaDeviceInfo deviceInfo);
        
		// Decoder will use any available Rocket-X's in a round-robin fashion to help
		// with decoding as much as possible before handing things off to the GPU.
        R3DStatus useRRXAsync(bool enabled);
