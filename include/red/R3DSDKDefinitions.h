...
struct ImageProcessingLimits
{
	...
	static const R3DSDK::ToneMap	OutputToneMapMap[];
	static const R3DSDK::ToneMap	OutputToneMapDefault;
...
// IPP2 only
typedef void * CreativeLutHandle;
...
public:
	...
	float					GainRed, GainGreen, GainBlue;// Ignored with ColorVersion3
	float					Saturation;					// Ignored with ColorVersion3
	float					Contrast;					// Ignored with ColorVersion3
	float					Brightness;					// Converted in ColorVersion3 mode.
	float					DRX;						// Highlight extension control. No effect for Dragon or Helium clips or in ColorVersion3 mode.
	...
	// Ignored with ColorVersion3 (IPP2).
	...
	float					Shadow;						// Ignored with ColorVersion3.
	float					FLUT;						// Fine ISO control. Ignored with ColorVersion3.
