...
public:
	...
	// True if Creative 3D LUT was set by user in-camera when clip was recorded.
	bool CameraCreativeLutPresent() const;
	...
	CreativeLutHandle LoadCreativeLut(const char * path) const;
	...
	// All decoder interfaces (Software, GPU & Rocket) can return per-frame
...
public:
	...
	// of threads will be used. When enableAutoRRX is true the decoder will
	// use any available Rocket-X's in a round-robin fashion to help with
	// decoding as much as possible before handing things off to the GPU.
	void Open(size_t noOfThreads = 0U, bool enableAutoRRX = true);
...
// 	2) try to load as a full path
// 	3) try to load from current directory
// If the 3D LUT is a sidecar file then it's important to call Clip::LoadCreativeLut()
// version of this API instead! See description of that API to get more information.
CreativeLutHandle LoadCreativeLut(const char * lutPath, const char * clipPath = NULL);

// Unload creative 3D LUTs loaded through one of the LoadCreativeLut() APIs. The supplied
// handle will be set to NULL for you. Can be safely called on R3D/RMD auto loaded 3D LUTs.
void UnloadCreativeLut(CreativeLutHandle * handle);

// Get the path for the given Creative 3D LUT. Returns NULL if handle is invalid.
// If this is a sidecar Creative 3D LUT then only the filename will be returned.
const char * GetCreativeLutPath(CreativeLutHandle handle);
